# 爬取好友的说说信息
#安装requests,selenium，pymysql包，安装在python的script文件夹下
...
def login():
    ...
    driver.find_element_by_id('u').send_keys('1034644152')
    driver.find_element_by_id('p').clear()
    driver.find_element_by_id('p').send_keys('xiaokeai1205')
...
def in_mysql(time, content, conn):
    time = str(time)
    content = str(content)
    conn = pymysql.Connect(host='localhost', port=3306, user='root', password='123456', db='qq_spider', charset='utf8')
...
def get_allQQ(mysession, g_tk, qzonetoken):
    # 获取好友QQ的网址
    url_friend = 'https://user.qzone.qq.com/proxy/domain/r.qzone.qq.com/cgi-bin/tfriend/friend_ship_manager.cgi?uin=1034644152&do=1&fupdate=1&clean=1&g_tk=' + str(
    ...
    for i in range(0, int(2)):
        ...
        try:
            ...
            for c, t in zip(content_list, time_list):
                c = c.replace('[{"con":', '')
                conn = pymysql.Connect(host='localhost', port=3306, user='root', password='123456', db='qq_spider',
                                       charset='utf8')
                in_mysql(t, c, conn)
